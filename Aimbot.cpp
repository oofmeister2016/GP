...
std::vector<int> GetHitboxesToScan(CBaseEntity* pEntity)
{
...
int GetCurrentPriorityHitbox(CBaseEntity* pEntity)
{
	...
	{
		return Clientvariables->Ragebot.Hitbox;
...
{
	return GameUtils::GetMultiplePointsForHitbox(pTarget, GetCurrentPriorityHitbox(pTarget), BoneMatrix);//pelvis
}
...
Vector RunAimScan( CBaseEntity* pTarget, float &simtime, Vector& origin )
{
	...
	if (Clientvariables->Ragebot.PositionAdjustment)
	{
		...
		for (int j = g_BacktrackHelper->PlayerRecord[pTarget->Index()].records.size() - 1; j >= 0; j--) //works pretty good for nospread
		{
			...
			if (record.m_flSimulationTime == g_BacktrackHelper->PlayerRecord[pTarget->Index()].records.at(j).m_flSimulationTime) {
...
void CAimbot::CompensateInaccuracies()
{
	if (G::UserCmd->buttons & IN_ATTACK)
	{
		...
		if (Clientvariables->Ragebot.NoSpread)
		{
