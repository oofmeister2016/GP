...
void CDraw::Init()
{
	...
	g_pSurface->SetFontGlyphSet(font_esp, XorStr("Helvetica"), 12, 450, 0, 0, FONTFLAG_OUTLINE);

	font_espnum = g_pSurface->SCreateFont();
	g_pSurface->SetFontGlyphSet(font_espnum, XorStr("Helvetica"), 9, 500, 0, 0, FONTFLAG_OUTLINE);

	font_espinfo = g_pSurface->SCreateFont();
	g_pSurface->SetFontGlyphSet(font_espinfo, XorStr("Terminal"), 14, FW_NORMAL, 0, 0, FONTFLAG_OUTLINE | FONTFLAG_DROPSHADOW);

	font_build = g_pSurface->SCreateFont();
	g_pSurface->SetFontGlyphSet(font_build, XorStr("Verdana"), 25, 750, 0, 0, FONTFLAG_DROPSHADOW);
}
