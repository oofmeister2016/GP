...
//	static DWORD m_dwOcclusionArray = *(uintptr_t*)(FindPattern("client.dll", (PBYTE) "\xA1\x00\x00\x00\x00\x8B\xB7\x00\x00\x00\x00\x89\x75\xF8", "x????xx????xxx", 0, 0) + 0x1); //"A1 ? ? ? ? 8B B7 ? ? ? ? 89 75 F8"
//	static DWORD m_bDidCheckForOcclusion = *(uintptr_t*)(FindPattern("client.dll", (PBYTE) "\xA1\x00\x00\x00\x00\x8B\xB7\x00\x00\x00\x00\x89\x75\xF8", "x????xx????xxx",  0) + 0x7); //"A1 ? ? ? ? 8B B7 ? ? ? ? 89 75 F8"
//
...
//
//	if (iHitbox == 0 && pBaseEntity->GetEyeAngles().x >= 66 && Clientvariables->Ragebot.AutomaticResolver)
//	{
//		trace_t tr;
//		Ray_t ray;
...
//
//	if (!Clientvariables->Ragebot.Multipoint)
//		return vPoints;
//
//	float rad = (untransformedBox->radius / 2) * (Clientvariables->Ragebot.Headscale / 100);
//
...
std::vector<Vector> GameUtils::GetMultiplePointsForHitbox( CBaseEntity* pBaseEntity, int iHitbox, VMatrix BoneMatrix[ 128 ] )
{
	...
	default:
		...
		vecArray[1] += Right * (hitbox->radius * Clientvariables->Ragebot.Headscale);
		vecArray[2] += Left * (hitbox->radius * Clientvariables->Ragebot.Headscale);
		break;
	}
	return vecArray;
}
