	...
	if (Clientvariables->Visuals.SpreadCrosshair)
	{
		...
		if (G::LocalPlayer && G::LocalPlayer->isAlive())
		{
			...
			if (cone > 0.0f)
			{
				...
				DrawFilledCircle(Vector2D(MidX, MidY), color, (int)size, 140);	
			}
		}
	}
}
...
void CESP::DrawLinesAA() {
	...
	g_pSurface->SetDrawColor(Color(0, 0, 255, 255));
	g_pSurface->DrawLine(src.x, src.y, dst.x, dst.y);
	g_Draw.StringA(g_Draw.font_esp, true, dst.x, dst.y, 0,0,255,255, "LBY");
}
...
void CESP::DrawPlayer(CBaseEntity* pPlayer, CBaseEntity* pLocalPlayer)
{
	...
	if (GameUtils::WorldToScreen(m_position, footpos) && GameUtils::WorldToScreen(m_top_position, headpos))
	{
		...
		if (Clientvariables->Visuals.Armor)
		{
			if (pPlayer->GetArmor() > 0)
			{
				...
				if (pPlayer->HasHelmet())
					g_Draw.StringA(g_Draw.font_espinfo, false, x + width + 3, y, 255, 255, 255, flPlayerAlpha[pPlayer->GetIndex()], "H | K");
				else
		...
		{
			if (pPlayer->IsScoped())
				g_Draw.StringA(g_Draw.font_espinfo, false, x + width + 3, y + 20, 255, 255, 255, flPlayerAlpha[pPlayer->GetIndex()], "SCOPED");
		}

		if (Clientvariables->Visuals.Fake)
		{
			if (G::FakeDetection[pPlayer->Index()] == 1)
				g_Draw.StringA(g_Draw.font_espinfo, false, x + width + 3, y + 10, 255, 255, 255, flPlayerAlpha[pPlayer->GetIndex()], "FAKE LBY"); // High delta breaker
			else if (G::FakeDetection[pPlayer->Index()] == 2)
				g_Draw.StringA(g_Draw.font_espinfo, false, x + width + 3, y + 10, 255, 255, 255, flPlayerAlpha[pPlayer->GetIndex()], "LBY"); // Low delta
		}
