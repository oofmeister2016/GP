...
void __fastcall Hooks::scene_end(void* thisptr, void* edx) {
	...
	if (Clientvariables->Visuals.ChamsEnable)
	{
		...
		static IMaterial* Fake = g_MaterialHelper->CreateMaterial(false, false, true);
		...
		for (int i = 1; i < g_pEngine->GetMaxClients(); ++i) {
			...
			if (ent == G::LocalPlayer && G::LocalPlayer != nullptr)
			{
				if (G::LocalPlayer->isAlive())
				{
					...
					{
						Vector OrigAng;
						OrigAng = G::LocalPlayer->GetEyeAngles();

						if (Clientvariables->Visuals.FakeAngleGhost == 1)
							G::LocalPlayer->SetAngle2(Vector(0, G::FakeAngle.y, 0));
						else
							G::LocalPlayer->SetAngle2(Vector(0, G::LocalPlayer->LowerBodyYaw(), 0));

						g_pRenderView->SetColorModulation(Clientvariables->Colors.FakeAngleGhost);
						g_pModelRender->ForcedMaterialOverride(Fake);
						G::LocalPlayer->draw_model(0x1, 195);
